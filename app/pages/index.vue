...
import type { Review } from '~/composables/useDb'
...
const { books: allBooks, reviews: allReviews } = useLibrary()
const allBookTags = useLiveQuery(() => db.bookTags.toArray(), [])
const seriesWithNew = useLiveQuery(
  () => db.series.where('newReleaseAvailable').equals(1).toArray(),
  []
)
