import { db, type Book, type Review } from './useDb'
...
export function useExport() {
  async function exportJson() {
    const [books, series, reviews, tags, bookTags] = await Promise.all([
      db.books.toArray(),
      db.series.toArray(),
      db.reviews.toArray(),
      db.tags.toArray(),
    ...
    const data = { books, series, reviews, tags, bookTags }
    const date = new Date().toISOString().split('T')[0]
    triggerDownload(JSON.stringify(data, null, 2), `bookshelf-export-${date}.json`, 'application/json')
  }

  async function exportHtml() {
    const books = await db.books.where('status').equals('read').toArray()
    const reviews = await db.reviews.toArray()
    const reviewMap = new Map<string, Review>(reviews.map(r => [r.bookId, r]))
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .review-text { font-size: 0.875rem; color: var(--text); line-height: 1.5; margin-top: 0.5rem; }
...
<footer>Exported from Bookshelf app</footer>
</body>
</html>`
