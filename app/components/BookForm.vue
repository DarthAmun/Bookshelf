...
import type { Book, Series } from '~/composables/useDb'
...
<template>
  <form class="space-y-4" @submit.prevent="emit('submit')">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Position in series</label>
        <input
    ...
    <div>
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Amazon URL</label>
      <input
    ...
    <slot name="actions">
      <button
        type="submit"
        class="w-full py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-medium text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
      >
        Save to library
      </button>
    </slot>
