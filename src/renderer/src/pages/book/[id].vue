...
import type { Book, Review, Tag } from '~/types'
...
const review = ref<Review | undefined>(undefined)
...
<template>
  <div v-if="book" class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
      ...
      <nav class="text-sm text-muted">
        <RouterLink v-if="series" :to="`/series/${series.id}`" class="hover:underline">{{ series.name }}</RouterLink>
  ...
  <div v-else class="text-center py-20 text-faint">
    <p>Book not found.</p>
    <RouterLink to="/" class="mt-2 text-brass hover:text-brass-soft text-sm">← Back to library</RouterLink>
  </div>
</template>
